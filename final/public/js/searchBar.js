        ...
        switch(this.type) {
            case "item":
                searchResultItem.click(e => {
                    window.location.assign(`item?name=${this.name}`);
                });
                break;
            case "pokemon":
                searchResultItem.click(e => {
                    window.location.assign(`pokemon?name=${this.name}`);
                });
                break;
            case "unknown":
...
$(() => {
    ...
    navSearchFormInput.on('keypress', (e) => {
        if(e.which == 13) {
            ...
            $.post('/search', {searchTerm: searchText});
            window.location.assign(`/search?searchTerm=${searchText}&page=1`);
        }
    });

    navSearchFormInput.click(e => {
        ...
        $.get('/search/history', (data) => {
            ...
            if (searchHistory.length > 0) {
                ...
                for (let i=0; i < 5; i++) {
                    ...
                    mostRecentSearches.push(searchHistory[i]);
                }

                searchHistoryList = $("<ul></ul>").addClass("searchHistoryList");

                mostRecentSearches.forEach(item => {
                    const searchTerm = item["searchTerm"];
                    const timestamp = item["timestamp"];

                    const searchHistoryItem = $("<li></li>").addClass("searchHistoryItem");
                    const searchHistoryItemLink = $("<a></a>").attr("href", `/search?searchTerm=${searchTerm}&page=1`).html(searchTerm);
                    searchHistoryItem.append(searchHistoryItemLink);
                    searchHistoryList.append(searchHistoryItem);
                });

                $("#searchContainer").append(searchHistoryList);
            }
        });
    });

    $("#searchInput").focusout(e => {
        if (searchHistoryList !== null && $(".searchHistoryList").is(':hover') != true) {
...
$("#nextBtn").click((e) => {
    ...
    window.location.assign(`/search?searchTerm=${searchText}&page=${pageNumber}`);
});
